...
import { BERSERK_CHARACTERS } from './data/berserk-characters.mjs';

const __filename = fileURLToPath(import.meta.url);
const __dirname = path.dirname(__filename);
const rootDir = path.resolve(__dirname, '..');

const avatarsDir = path.join(rootDir, 'public', 'avatars', 'berserk');
const charactersJsonPath = path.join(rootDir, 'src', 'data', 'animes', 'berserk', 'characters.json');

fs.mkdirSync(avatarsDir, { recursive: true });
fs.mkdirSync(path.dirname(charactersJsonPath), { recursive: true });

async function fetchWikiThumbnail(title) {
  const url = `https://berserk.fandom.com/api.php?action=query&prop=pageimages&titles=${encodeURIComponent(title)}&pithumbsize=800&format=json&redirects=1`;
  try {
    const res = await fetch(url, {
      headers: {
        'User-Agent': 'AnimedleBot/1.0 (Windows NT 10.0; Win64; x64)',
        'Referer': 'https://berserk.fandom.com/'
      }
    });
...
async function buildBerserk() {
  console.log(`\n=== INICIANDO INTEGRAÇÃO DE BERSERK (${BERSERK_CHARACTERS.length} PERSONAGENS) ===\n`);

  let successCount = 0;
  const missing = [];

  for (let i = 0; i < BERSERK_CHARACTERS.length; i++) {
    const char = BERSERK_CHARACTERS[i];
    const outputPath = path.join(avatarsDir, `${char.id}.png`);
    const wikiTitle = char.wikiTitle || char.name;

    console.log(`[${i + 1}/${BERSERK_CHARACTERS.length}] Processando ${char.name} (${char.id}) [Wiki: ${wikiTitle}]...`);

    let imageUrl = await fetchWikiThumbnail(wikiTitle);

    // Tentativa alternativa com char.name se wikiTitle falhar
  ...
  const cleanCharacters = BERSERK_CHARACTERS.map(c => {
...
buildBerserk().catch(console.error);
