...
import { ALL_AOT_CHARACTERS } from './data/aot-characters.mjs';

const __filename = fileURLToPath(import.meta.url);
const __dirname = path.dirname(__filename);
const rootDir = path.resolve(__dirname, '..');

const avatarsDir = path.join(rootDir, 'public', 'avatars', 'attack-on-titan');
const charactersJsonPath = path.join(rootDir, 'src', 'data', 'animes', 'attack-on-titan', 'characters.json');

fs.mkdirSync(avatarsDir, { recursive: true });
fs.mkdirSync(path.dirname(charactersJsonPath), { recursive: true });

async function fetchWikiThumbnail(title) {
  const url = `https://attackontitan.fandom.com/api.php?action=query&prop=pageimages&titles=${encodeURIComponent(title)}&pithumbsize=800&format=json&redirects=1`;
  try {
    const res = await fetch(url, {
      headers: {
        'User-Agent': 'AnimedleBot/1.0 (Windows NT 10.0; Win64; x64)',
        'Referer': 'https://attackontitan.fandom.com/'
      }
    });
...
async function buildAttackOnTitan() {
  console.log(`\n=== INICIANDO INTEGRAÇÃO DE ATTACK ON TITAN (${ALL_AOT_CHARACTERS.length} PERSONAGENS) ===\n`);

  let successCount = 0;

  for (let i = 0; i < ALL_AOT_CHARACTERS.length; i++) {
    const char = ALL_AOT_CHARACTERS[i];
    const outputPath = path.join(avatarsDir, `${char.id}.png`);
    const wikiTitle = char.wikiTitle || char.name;

    console.log(`[${i + 1}/${ALL_AOT_CHARACTERS.length}] Processando ${char.name} (${char.id}) [Wiki: ${wikiTitle}]...`);
  ...
  const cleanCharacters = ALL_AOT_CHARACTERS.map(c => {
...
buildAttackOnTitan().catch(console.error);
